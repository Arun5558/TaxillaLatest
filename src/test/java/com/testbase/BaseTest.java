...
import com.keywords.ApplicationKeywords;
import com.reports.ExtentManager;
// acceptable failure, critical failure, unexpected
import com.runner.DataUtil;
...
public class BaseTest {
	...
	public void beforeTest(ITestContext context) throws NumberFormatException,
			FileNotFoundException, IOException, ParseException {
		System.out.println("----------Before Test---------");
		String datafilpath = context.getCurrentXmlTest().getParameter(
				"datafilpath");
		String dataFlag = context.getCurrentXmlTest().getParameter("dataflag");
		String iteration = context.getCurrentXmlTest()
				.getParameter("iteration");
		String sheetName = context.getCurrentXmlTest()
				.getParameter("suitename");
		// suitename(sheetname)
		...
		// JSONObject data = new ReadingXLS().getTestData(sheetName, dataFlag,
		// (Integer.parseInt(iteration)+1), datafilpath);
		...
		// 1 app keyword object for entire test
		// -All @Test
		app.setReport(test);
	...
	public void beforeMethod(ITestContext context) {
		...
		String criticalFailure = (String) context
				.getAttribute("criticalFailure");
		if (criticalFailure != null && criticalFailure.equals("Y")) {
			test.log(Status.SKIP, "Critical Failure in Prevoius Tests");
			throw new SkipException("Critical Failure in Prevoius Tests");
			// skip
			// in
			// testNG
		}
